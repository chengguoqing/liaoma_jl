...
Page({
  ...
  get_mx() {
    ...
    base.ajax("x_get_attention_list", x_get_transfer_list, function(data) {
      data.data.map(a => {
  ...
  delect_er(e) {
    var id_r = e.currentTarget.dataset.idr,
    ...
    wx.showModal({
      ...
      success: function(res) {
        if (res.confirm) {
          ...
          base.ajax("x_remove_attention", x_remove_attention, function(data) {
            sd_kis.splice(idx, 1);
            th.setData({
              sd_kis: sd_kis
            })
